...
__global__ void transform(float *matrix, float *matrix2){
    for(unsigned int global_id = blockIdx.x * blockDim.x + threadIdx.x;global_id<N1*Height*Width;global_id+=gridDim.x * blockDim.x){
        ...
        unsigned int c = global_id / (Height*Width);
        unsigned int hw = global_id % (Height*Width);
        int h = (hw)/Width+1;
        int w = (hw)%Width+1;
...
__global__ void conv2d(float * __restrict__ values,const float * __restrict__ kernel, float * __restrict__ outputs){
    ...
    for (unsigned int th = 0; th < TH; ++th) {
        for (unsigned int tw = 0; tw < TW; ++tw) {
            ...
            atomicAdd(&outputs[n*Height*Width+(h_start + th)*Width+w_start+tw],temp_result[(th * TW + tw)]);
...
int main(void){
    ...
    cudaMalloc(&device_K,N2*N1*sizeof(float)*9);
    cudaMemcpy(device_K,K,N1*N2*9*sizeof(float),cudaMemcpyHostToDevice);
    float *host_out_cudnn = new float[N2*Height*Width];
    float *host_out = new float[N2*Height*Width];
    Conv conv_cudnn;
    conv_cudnn.initialize(1,N1,Height,Width,N2,1,3,3,1);
    float * out;
    out = conv_cudnn.forward(device_input);
    ...
    cudaMemcpy(host_out_cudnn,out,N2*Height*Width*sizeof(float),cudaMemcpyDeviceToHost);
    view(host_out_cudnn);
    float *matrix;
    ...
    cudaMemset(device_out,0,N2*Width*Height*sizeof(float));
    transform<<<68,512>>>(device_input,matrix);
    conv2d<<<TCS*THS*TWS,N2>>>(matrix,device_K, device_out);
    cudaEventRecord(event_stop);
    cudaEventSynchronize(event_stop);
    float time3;
    cudaEventElapsedTime(&time3, event_start, event_stop);
    cudaMemcpy(host_out,device_out,N2*Height*Width*sizeof(float),cudaMemcpyDeviceToHost);
    view(host_out);
    cout<<N2<<","<<N1<<","<<Height<<","<<Width<<","<<TH<<","<<TW<<","<<TC<<","<<cuda_time<<","<<time3<<","<<check_diff(host_out_cudnn,host_out,N2*Height*Width)<<endl;
