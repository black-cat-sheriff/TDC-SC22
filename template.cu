...
#define N1 place holder
#define N2 place holder
#define N3 128
#define Channel 1
#define TC place holder
#define TCS ((N1-1)/TC + 1)
#define Height place holder
#define Width place holder
#define R 3
#define S 3
...
int main(void){
    ...
    Conv conv;
    conv.initialize(1,N1,Height,Width,N2,1,3,3,1);
    float *out_cudnn;
    float *out_cudnn_host = new float[N2*Height*Width];
    ...
    cudaMemcpy(out_cudnn_host,out_cudnn,N2*Height*Width*sizeof(float),cudaMemcpyDeviceToHost);
    ...
    chkerr(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&numBlks,conv2d,N2,0));
    chkerr(cudaGetLastError());
    chkerr(cudaDeviceSynchronize());
    float occupancy = (numBlks * N2 * 1.0f)/1024;
    cout<<N1<<","<<N2<<","<<Height<<","<<Width<<","<<TH<<","<<TW<<","<<
    TC<<","<<occupancy<<","<<time_cudnn<<","<<time3<<","<<check_diff(out_tdc,out_cudnn_host,N2*Height*Width)<<endl;
    return 0;
}
