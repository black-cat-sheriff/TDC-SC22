...
extern "C" __global__ void default_function_kernel0(float* __restrict__ data, float* __restrict__ kernel, float* __restrict__ compute) {
    ...
    for (int rc_outer = 0; rc_outer < 64; ++rc_outer) {
        for (int ry_outer = 0; ry_outer < 3; ++ry_outer) {
            for (int rx_outer = 0; rx_outer < 3; ++rx_outer) {
                pad_temp_shared[(0)] = (((((1 <= (((int)blockIdx.y) + ry_outer)) && ((((int)blockIdx.y) + ry_outer) < 57)) && (1 <= (((int)blockIdx.x) + rx_outer))) && ((((int)blockIdx.x) + rx_outer) < 57)) ? data[(((((((rc_outer * 3136) + (((int)blockIdx.y) * 56)) + (ry_outer * 56)) + ((int)blockIdx.x)) + rx_outer) - 57))] : 0.000000e+00f);
    ...
    compute[((((((int)blockIdx.z) * 3136) + (((int)blockIdx.y) * 56)) + ((int)blockIdx.x)))] = compute_local[(0)];
}
